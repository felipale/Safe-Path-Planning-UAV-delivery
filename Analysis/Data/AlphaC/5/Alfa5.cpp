...
float delta_val = 0.0f;                // δ fixo em 0
...
int main() {
    ...
    const int   trials                 = 5;      // número de trials = 5
    const int   horizon                = 5;

    // ── define e entra na pasta existente de saída ─────────────────────────
    const std::filesystem::path base_output =
        "C:\\Supaero\\Stage\\Codigo3\\Analysis\\Data\\AlphaC\\6";
    ...
    // ── executa simulação única com α=0.0035, β=0, δ=0 ─────────────────────
    const std::string run_label = "Default";
