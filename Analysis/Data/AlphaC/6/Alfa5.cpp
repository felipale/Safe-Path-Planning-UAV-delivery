...
float alpha_val = 0.0035f;             // α fixo em 0.0035
float beta_val  = 0.0f;                // β fixo em 0
float delta_val = 0.000065f;           // δ será varrido
...
int main() {
    ...
    const std::filesystem::path base_output =
        "C:\\Supaero\\Stage\\Codigo3\\Analysis\\Data\\AlphaC\\5";
    ...
    zero_vec.setZero(); one_vec.setZero(); one_vec(0) = 1.0f;
    ...
    for (float d = 0.000065f; d <= 0.00015f + 1e-9f; d += 0.000025f) {
        deltas.push_back(d);
    }
    ...
    // ── varre δ mantendo α e β fixos, salvando tudo em base_output ─────────
    for (auto d : deltas) {
        delta_val = d;

        std::ostringstream ss;
        ss << "Final" << std::fixed << std::setprecision(6) << d;
