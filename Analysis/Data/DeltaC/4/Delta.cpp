...
float *beta_ptr;                     // β, peso no ângulo
float *delta_ptr;                    // δ, peso na velocidade angular
const float alpha = 0.0035f;         // α, peso na velocidade linear
...
double cost_function(const Eigen::VectorXf& x) {
    ...
    return
      ...
      + alpha    * x(1)*x(1)          // α(ẏ−ẏ_ref)^2
      + (*beta_ptr)  * x(2)*x(2)      // β(β−β_ref)^2
...
int main() {
    ...
    const int    actions_state_possible = 7;
    const int    trials                 = 3;
    ...
    float beta_val  = 0.001f;         // β fixo em 0.001
    float delta_val = 0.000f;           // δ inicia em 0
    beta_ptr  = &beta_val;
    delta_ptr = &delta_val;

    // ── muda para a pasta de saída ────────────────────────────────────────
    std::filesystem::current_path(
      "C:\\Supaero\\Stage\\Codigo3\\Analysis\\Data\\Delta\\4"
    );
    ...
    // ── varredura de δ: 0.001 ≤ δ ≤ 0.009 em passos de 0.0005 ───────────────────
    int delta_idx = 1;
    for (delta_val = 0.000f;
         delta_val <= 0.001f + 1e-6f;
         delta_val += 0.0001f, ++delta_idx)
    {
