...
int main() {
    ...
    const auto base_output =
        std::filesystem::path("C:\\Supaero\\Stage\\Codigo3\\Analysis\\Data\\Cos");
    ...
    // ── inicializa o mapa de referência com cosseno e seno negativo ──────
    {
        Eigen::VectorXf axx(n_states);
        // percorre até max_time + dt para garantir que
        // goal(max_time + dt) exista internamente
        for (float t = 0.0f; t <= max_time + dt + 1e-6f; t += dt) {
            axx << std::cos(t), -std::sin(t), 0.0f, 0.0f;
    ...
    // ── executa simulação única com trajetória cossenoidal ──────────────
    const std::string run_label = "Cos";
