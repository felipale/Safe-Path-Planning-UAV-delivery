...
int main() {
    ...
    std::filesystem::current_path("C:\\Supaero\\Stage\\Codigo3\\Analysis\\Data\\BetaC\\4");
    ...
    // varredura macro: beta de 0.000 → 0.001 em passos de 0.001,
    // mas nomeando arquivos como Beta1, Beta2, ...
    int beta_idx = 1;
    for (beta_val = 0.000f; beta_val <= 0.001f + 1e-6f; beta_val += 0.0001f, ++beta_idx) {
        std::ostringstream ss;
        ss << "Beta" << beta_idx;
        std::cout << "Rodando β = " << std::fixed << std::setprecision(4)
