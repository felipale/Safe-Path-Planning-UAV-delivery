...
int main() {
    ...
    const int actions_state_possible = 6;  // reduzido de 7 → 6
    const int trials                 = 3;   // reduzido de 10 → 3
    ...
    std::filesystem::current_path("C:\\Supaero\\Stage\\Codigo3\\Analysis\\Data\\BetaC\\5");
    ...
    // varredura macro: beta de 0.0000 → 0.0001 em passos de 0.00002,
    // mas nomeando arquivos como Beta1, Beta2, ...
    int beta_idx = 1;
    for (beta_val = 0.0000f; beta_val <= 0.0001f + 1e-6f; beta_val += 0.00002f, ++beta_idx) {
        std::ostringstream ss;
        ss << "Beta" << beta_idx;
        std::cout << "Rodando β = " << std::fixed << std::setprecision(5)
