...
int main() {
    ...
    const int trials                 = 10; // aumentado de 3 → 10
    const int horizon                = 5;  // reduzido de 15 → 55
    ...
    std::filesystem::current_path("C:\\Supaero\\Stage\\Codigo3\\Analysis\\Data\\BetaC\\1");
    ...
    // varredura macro: beta de 0.0 → 0.9 em passos de 0.1,
    // mas nomeando arquivos como Beta1, Beta2, ...
    int beta_idx = 1;
    for (beta_val = 0.0f; beta_val <= 0.9f + 1e-6f; beta_val += 0.1f, ++beta_idx) {
