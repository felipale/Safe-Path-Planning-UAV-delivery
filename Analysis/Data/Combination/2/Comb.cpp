...
int main() {
    ...
    // ── pasta de saída ────────────────────────────────────────────────────
    std::filesystem::current_path("C:\\Supaero\\Stage\\Codigo3\\Analysis\\Data\\Combination\\2");
    ...
    for (alpha_val = 0.002f; alpha_val <= 0.004f + 1e-6f; alpha_val += 0.001f) {
        for (beta_val = 0.0000f; beta_val <= 0.0002f + 1e-9f; beta_val += 0.00005f) {
            for (delta_val = 0.005f; delta_val <= 0.02f + 1e-6f; delta_val += 0.005f) {
                ...
                // monta prefixo como A2m_B10m_D5m etc.
                std::string alpha_str = format_mili("A", alpha_val);
                std::string beta_str  = format_mili("B", beta_val);
                ...
                for (int trial = 0; trial < trials; ++trial) {
                    ...
                    fname << prefix << "_Data" << trial;

                    model1.startSimulation(
                        fname.str(),
                        max_time,
                        x0,
                        1,  // apenas uma trial por chamada
                        horizon,
                        stopping_criterion
                    );
                }
