...
int main() {
    ...
    const int trials                 = 1;   // apenas UMA simulação para gerar um arquivo
    ...
    std::filesystem::current_path("C:\\Supaero\\Stage\\Codigo3\\Analysis\\Data\\Alpha\\2");
    ...
    // ── varredura de α: 0.000 → 0.004 em passos de 0.002 (3 pontos) ────────
    int alpha_idx = 1;
    for (alpha_val = 0.001f; alpha_val <= 0.004f + 1e-9f; alpha_val += 0.0005f, ++alpha_idx) {
        std::ostringstream ss;
        ss << "Alfa" << alpha_idx;
        std::string base = ss.str();
        std::cout << "Rodando α = " << std::fixed << std::setprecision(3)
                  << alpha_val << " … (salvando em " << base << ".csv)\n";

        // 1 simulação → gera apenas base + "Data0.csv"
        model1.startSimulation(
            base,       // NameBase: Alfa1, Alfa2, …
            max_time,   // duração da simulação em segundos
            x0,         // estado inicial
            trials,     // apenas 1 trial
            horizon,    // horizonte de busca
            stopping_criterion
        );

        // renomeia "AlfaXData0.csv" → "AlfaX.csv"
