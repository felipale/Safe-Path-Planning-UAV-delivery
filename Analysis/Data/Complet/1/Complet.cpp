...
float alpha_val;                      // α (peso da velocidade linear)
...
int main() {
    ...
    const std::filesystem::path base_output =
      "C:\\Supaero\\Stage\\Codigo3\\Analysis\\Data\\Complet\\1";
    ...
    std::vector<float> alphas = {0.0028f, 0.0030f, 0.0032f};
    std::vector<float> deltas = {0.0045f, 0.0050f, 0.0055f};
    ...
    for (auto a : alphas) {
        ...
        for (auto d : deltas) {
            ...
            auto outdir = base_output / ss.str();

            std::filesystem::create_directories(outdir);
            std::filesystem::current_path(outdir);
            std::cout << "Salvando em: " << outdir << "\n";
