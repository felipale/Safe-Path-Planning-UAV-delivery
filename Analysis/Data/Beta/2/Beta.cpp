...
int main() {
    ...
    const int trials                 = 3;   // reduzido de 5 → 3
    const int horizon                = 15;  // reduzido de 25 → 15
    ...
    std::filesystem::current_path("C:\\Supaero\\Stage\\Codigo3\\Analysis\\Data\\Beta\\2");
    ...
    // varredura macro: beta de 0.0 → 0.5 em passos de 0.1,
    // mas nomeando arquivos como Beta1, Beta2, ...
    // --- varredura de beta de 0.1 até 0.9 em passos de 0.05 ---
    int beta_idx = 1;
    for (beta_val = 0.1f; 
         beta_val <= 0.9f + 1e-6f; 
         beta_val += 0.05f, ++beta_idx) 
    {
        ...
        std::cout << "Rodando β = " 
                  ...
                  << beta_val 
                  << " … (salvando em " << ss.str() << ".csv)\n";

        model1.startSimulation(
            ss.str(),      // NameBase: Beta1, Beta2, …
            5.0f,          // max_time (s)
            x0,            // estado inicial
