...
int main() {
    ...
    std::filesystem::current_path("C:\\Supaero\\Stage\\Codigo3\\Analysis\\Data\\Beta\\4");
    ...
    // ── varredura de β: 0.0001 ≤ β ≤ 0.002 em passos de 0.0001 ────────────
    int beta_idx = 1;
    for (beta_val = 0.0001f; beta_val <= 0.002f + 1e-9f; beta_val += 0.0001f, ++beta_idx) {
        ...
        model1.startSimulation(
            base,      // NameBase: Beta1, Beta2, …, Beta20
            max_time,  // duração da simulação
            x0,        // estado inicial
            trials,    // numero de trials (3)
            horizon,
            stopping_criterion
        );
