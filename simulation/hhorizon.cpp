// -----------------------------------------------------------------------------
// Este arquivo define funções auxiliares e executa uma simulação com horizonte
// variável de planejamento, semelhante a `Horizon.cpp`, mas utilizando:
//
// - Uma função de custo simplificada que considera apenas erro de posição.
// - Um mapa `Goal` como função do tempo (referência).
//
// Pode ser usado para testar a sensibilidade do controle ao horizonte,
// removendo o termo de velocidade do custo para observar diferenças.
// -----------------------------------------------------------------------------
...
double cost_function(const Eigen::VectorXf& x){
   ...
   // Custo apenas em função da posição — ignora velocidade
...
Eigen::VectorXf goal(double current_time){ // Retorna a meta (goal) de acordo com o tempo atual
