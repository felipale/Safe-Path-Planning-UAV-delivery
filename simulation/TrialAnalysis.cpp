// -----------------------------------------------------------------------------
// Este arquivo realiza a análise de desempenho do algoritmo de controle,
// registrando o tempo de execução ou custo de cada tentativa de simulação.
// 
// - `cost_function`: custo baseado em posição e velocidade.
// - `goal`: objetivo fixo (posição = 1, velocidade = 0).
// - `writeDataToCSV`: salva os resultados em arquivos CSV para análise posterior.
// 
// O `main()` executa as simulações e coleta dados sobre os tempos/custos das
// execuções com diferentes parâmetros ou configurações.
// -----------------------------------------------------------------------------
#include "Model.h"
#include <iomanip> // Para definir precisão de saída numérica

// Função de custo quadrática (posição e velocidade)
...
// Objetivo fixo: posição = 1, velocidade = 0
...
// Critério de parada desativado — retorna sempre falso
...
// Função para salvar um vetor de dados (ex: tempo de execução) em CSV
void writeDataToCSV(vector<double> a , string filename){
            ofstream file(filename);
            if (file.is_open()) {
                // Escreve cada valor do vetor com alta precisão
...
int main(){
    ...
    const int p = 2;        // Número de saídas
    
    // Definição do modelo dinâmico
    ...
    noise<< 0.04;           // ruído aplicado nas entradas
    int actions_state_possible = 10;
    float discrete_time_step = 0.03;
    discretization_actions << -0.2, 0.2, 0.04;
    Eigen::VectorXf x_0(n); // Estado inicial
    x_0 << 0,0;
    
    // Instancia o modelo de simulação
    ...
    vector<double> ax_one_horizon;  // Vetor para guardar tempos por tentativa
    string name ;
    stringstream ss;

    // Nó inicial para os trials
    ...
    // Loop por horizontes de predição de 1 a 24
    for (horizon = 1; horizon<=24; horizon++ ){
            inicial = chrono::high_resolution_clock::now();
        
        // Executa 1000 trials para esse horizonte
        ...
        // Limpa vetor e mapa para próxima rodada
        ax_one_horizon.clear();
        model1.clear_all_exploited(all_exploited);

    }
    // Mostra a resolução do clock
